...
public class ItemBuilder {
    ...
    public ItemBuilder skullOwner(String playerName){
        ...
        if (itemMeta != null) {
            Bukkit.getAsyncScheduler().runNow(KitPreview.getInstance(), e -> itemMeta.setOwnerProfile(Bukkit.getOfflinePlayer(UUID.fromString(playerName)).getPlayerProfile()));
        }
        ...
        if (itemMeta != null) {
            player.getPlayerProfile()
                    ...
                    .thenAcceptAsync(itemMeta::setOwnerProfile,
                            runnable -> Bukkit.getScheduler().runTask(KitPreview.getInstance(), runnable));
        }
    ...
    public ItemBuilder addContainerValue(String key, String value){
        ItemMeta meta = itemStack.getItemMeta();
        if(meta != null){
            meta.getPersistentDataContainer().set(new NamespacedKey(KitPreview.getInstance(), key), PersistentDataType.STRING, value);
            itemStack.setItemMeta(meta);
        }
        return this;
    }
    ...
    public ItemBuilder addContainerValue(String key, int value){
        ItemMeta meta = itemStack.getItemMeta();
        if(meta != null){
            meta.getPersistentDataContainer().set(new NamespacedKey(KitPreview.getInstance(), key), PersistentDataType.INTEGER, value);
            itemStack.setItemMeta(meta);
        }
        return this;
    }

    public ItemBuilder addContainerValue(JavaPlugin plugin, String key, boolean value){
        ItemMeta meta = itemStack.getItemMeta();
        if(meta != null){
